...
<?php $FileRunWebCond = file_get_contents ('/etc/nginx/conf.d/filerun.conf'); preg_match_all('/(?<=\blisten )\S+/is', $FileRunWebCond, $FileRunPort);?>
...
<span class="float-right badge text-info"><?php passthru('sudo /opt/de_GWD/ui-checkEditionARM');?></span>
    ...
    <ul class="navbar-nav ml-auto ml-md-0">
      <li class="nav-item no-arrow mx-1" style="display:<?php if ($checkFileRun === true) echo 'block'; else echo 'none';?>">
        <a class="nav-link" href="javascript:void(0)" onclick="window.open(location.origin+':<?php echo $FileRunPort[0][0]; ?>')">
          ...
          <div id="xDNSsBody" class="card-body" style="display:<?php if ($xDNSsConf === true) echo 'block'; else echo 'none';?>">
            <div class="form-row">
              ...
              <div class="col-md-3 input-group my-2">
                ...
                <input type="text" class="form-control" value="<?php echo $de_GWDconf->FORWARD->domain ?>:<?php echo $de_GWDconf->FORWARD->xDNSs->port ?>" READONLY>
              </div>
            </div>
          </div>
        </div>
...
<div id="FWD1qrpop$FWD1num" class="modal fade" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title mx-auto" >默认中转线二维码 - $FWD1mark</h5>
      </div>
      <div class="modal-body">
...
for( let i = 0; i<len; i++){
  ...
  $('#FWD1upstreamDrop').append("<a class='dropdown-item' href='#' id='nodeName"+i+"'>"+name+"</a>");
  $('#nodeName'+i).click(function(){ $('#FWD1upstream').html(name); $('#FWD1upstream').val(address)})
}
})
...
for( let i = 1; i<=len; i++){
    var RproxyS0uuid = $('#RproxyS0uuid'+i).val()
