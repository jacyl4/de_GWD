  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <button id="adBlistURLload" type="button" class="btn btn-secondary btn-sm col-12 justify-content-center" style="border-Radius: 0px;">清空内容</button>
        <textarea id="adBlistURL" class="form-control" style="border-Radius: 0px;" rows="5" placeholder="广告域名"><?php print(implode("\n", $de_GWDconf->dns->adBlistURL)) ?></textarea>
        ...
        <button id="adWlistURLload" type="button" class="btn btn-secondary btn-sm col-12 justify-content-center" style="border-Radius: 0px;">加载预置</button>
        <textarea id="adWlistURL" class="form-control" style="border-Radius: 0px;" rows="5" placeholder="非广告域名"><?php print(implode("\n", $de_GWDconf->dns->adWlistURL)) ?></textarea>
      </div>
      <div class="modal-footer">
        <button id="buttonADlistsURL" type="button" class="btn btn-outline-secondary btn-sm">
...
<div class="input-group input-group-sm mb-3">
  <div class="input-group-prepend">
    <span class="input-group-text">本地局域网分流 & BT</span>
  </div>
...
$.get('./act/NodeSMrules.php', {nodeSMshowYoutube:nodeSMshowYoutube, nodeSMshowNetflix:nodeSMshowNetflix, nodeSMshowHDH:nodeSMshowHDH, nodeSMshowTVB:nodeSMshowTVB, nodeSMshowBahamut:nodeSMshowBahamut, nodeSMshowApple:nodeSMshowApple, nodeSMshowSteam:nodeSMshowSteam}, function(result){
...
var regexRuleLoadDefault=$.ajax({url:"/act/ADlistDefaultRegex",async:false})
$("#regexRule").val(regexRuleLoadDefault.responseText)
})

$('#adlistURLload').click(function(){
var adlistURLloadDefault=$.ajax({url:"/act/ADlistDefault",async:false})
$("#adlistsURL").val(adlistURLloadDefault.responseText)
})

$('#adBlistURLload').click(function(){
$("#adBlistURL").val("")
})

$('#adWlistURLload').click(function(){
var adWlistURLloadDefault=$.ajax({url:"/act/ADwlistDefault",async:false})
$("#adWlistURL").val(adWlistURLloadDefault.responseText)
})
...
adBlistURL=$('#adBlistURL').val()
adWlistURL=$('#adWlistURL').val()
$.get("./act/submitADList.php", {regexRule:regexRule, adlistsURL:adlistsURL, adBlistURL:adBlistURL, adWlistURL:adWlistURL}, function(result){
...
let steamNum = SMarray[12]
let steamName = SMarray[13]
if (steamName == "-none-") {
    steamName = " 直连 "
}
